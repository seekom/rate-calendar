...
this.htmlHdrRow;if(this.ShowRateName)html+=this.htmlHdrColName.replace("{ColWidths.Name}",this.ColWidths.Name)
...
{var roomHtml=room.Desc==''?room.Name:this.htmlRoomDetails.replace("{RoomDesc}",room.Desc).replace("{RoomName}",room.Name);var rateHtml=rate.Desc==''?rate.Name:this.htmlRateDetails.replace("{RateDesc}",rate.Desc).replace("{RateName}",rate.Name);html+=this.htmlColName.replace("{MinStaysCount}",rate.MinStays.length).replace("{RoomDetails}",roomHtml).replace("{RateDetails}",rateHtml)+
...
{var availClass=dayInfo.Status=='Instant Confirmation'?'available':'onreq';var availInfo=dayInfo.Available+' available<br>'+'Per night rate - '+rate.BasePax+' person'+(rate.BasePax>1?'s':'')+'<br>'+'Minimum stay - '+dayInfo.MinStay+' night'+(dayInfo.MinStay>1?'s':'');var roomRate=data.CurrencySymbol+Math.round(parseInt(dayInfo.BaseRate));var dayDetails=this.htmlDayDetails.replace("{DateYmd}",this.DateToString(day)).replace("{MinStay}",dayInfo.MinStay);dayDetails=dayDetails.replace("{RoomId}",room.Id).replace("{RateId}",rate.Id);dayDetails=dayDetails.replace("{CustomUrl}",data.CustomUrl);dayDetails=dayDetails.replace("{AvailabilityInfo}",availInfo).replace("{RoomRate}",roomRate);html+=this.htmlColDayAvail.replace("{AvailClass}",availClass).replace("{OddEven}",rowType).replace("{DayDetails}",dayDetails);}
