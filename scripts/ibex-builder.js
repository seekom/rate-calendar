// If there is no console, it will create a console function
	...
	// 1. Initialises the datepicker including setting its "onHideStart" event to reload the Seekom data following a date change
	...
	// when the user clicks the Prev or Next buttons or clicks a day cell to load the Seekom booking page
	...
	ibexBuilder.Initialise = function()
	{
		...
		this.htmlHdrColName = '<td width="{ColWidths.Name}">Room Type/Package<br><i>Rate Name</i></td>';
		this.htmlHdrColGuests = '<td width="{ColWidths.Guests}" class="center">Max<br>Guests</td>';
		...
		this.htmlRoomDetails = '<a class="addToolTip" title="<div id=\'ToolTipTextWrap\'>Room</div>{RoomDesc}">{RoomName}</a>';
	 ...
	 *
	 * @param SeekomRooms data the SeekomRooms object containing the retrieved data
	 */
	ibexBuilder.Build = function(data)
	{
		...
		// RateName column present only if ShowRateName option selected
		if (this.ShowRateName) html += this.htmlHdrColName.replace("{ColWidths.Name}", this.ColWidths.Name)
		...
		for (var roomKey = 0; roomKey < data.Rooms.length; roomKey++)
		{
			...
			for (var rateKey = 0; rateKey < room.Rates.length; rateKey++)
			{
				...
				for (var minStayKey = 0; minStayKey < rate.MinStays.length; minStayKey++)
    			{
					...
					if (roomRateRow)
					{
						var roomHtml = room.Desc == '' ? room.Name : this.htmlRoomDetails.replace("{RoomDesc}", room.Desc).replace("{RoomName}", room.Name);
						var rateHtml = rate.Desc == '' ? rate.Name : this.htmlRateDetails.replace("{RateDesc}", rate.Desc).replace("{RateName}", rate.Name);
					...
					for (var dayCt = 0; dayCt < this.NoOfDays; dayCt++)
					{
						...
						else
						{
							...
							var availInfo = dayInfo.Available + ' available<br>' +
										    'Per night rate - ' + rate.BasePax + ' person' + (rate.BasePax > 1 ? 's' : '') + '<br>' +
										    'Minimum stay - ' + dayInfo.MinStay + ' night' + (dayInfo.MinStay > 1 ? 's' : '');
